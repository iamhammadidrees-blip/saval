...
File A upload
  ...
  → filterPending()       (color / keywords)
  → toPendingParts()
  → IndexedDB + Zustand   (pendingParts)
        │
        ├─→ Pending table          (raw pendingParts)
        ├─→ aggregateRequired()    → Required table
        │         │
...
| Color first | orange / yellow / lightgreen → keep; green → drop |
| Keyword fallback | if color none: pending keywords → keep; resolved / PK-… → drop |
| Else | drop (not pending) |
...
pendingParts
  → aggregateRequired()
  → Required table / Download File B
```
...
|------|--------|-------------------|
| Pending | File A → status filter | none (row-level) |
